 ...
 * @Last Modified Date: 2024-07-17 12:29:58
...
// 在进入战斗界面之前复制游戏的存档
AOnBeforeScript({
    if (completedFlags % 100 == 0 && completedFlags != 0) {
        Logger.Debug(SetRandomSeed());
    }
...
void AScript()
{
    ...
    auto condition = [=]() -> bool {
        ...
        auto allPtrs = MergeVectors<APlant*>(melon_ptrs, gloom_ptrs, flower_ptrs, leaf_ptrs);
        ...
        std::vector<AGrid> ladderList = {{2, 2}, {5, 2}, {2, 3}, {5, 3}};
        for (const auto& [row, col] : ladderList) {
            if (!Judge::IsExistLadder(row, col)) {
#ifdef MULTITEST
                auto str = std::format("第 {} 轮 丢失梯子，进行第 {} 次SL", 2 * cresc::GetCurrentLevel(), loadCount + 1);
                SaveLoad(str);
                consoleLogger.Error(str);
#endif
                return false;
    ...
    auto callback = [=] { SetZombieIsStealth(false); };

#ifdef MULTITEST
    SetZombieIsStealth(true);
...
#else
    AConnect(AKey('T'), [=] { ASkipTick(condition, callback); });
#endif
}
